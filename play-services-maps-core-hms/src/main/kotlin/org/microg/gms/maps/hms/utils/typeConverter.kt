...
fun GmsPatternItem.toHms(): PatternItem {
    return when (type) {
        TYPE_DASH -> Dash(length)
        TYPE_DOT -> Dot()
        TYPE_GAP -> Gap(length)
        else -> PatternItem(type, length)
    }
}
...
fun GoogleMapOptions.toHms(): HuaweiMapOptions {
    ...
    Log.d("gmsmaptype", "liteMode:$liteMode")
...
fun GmsTileOverlayOptions.toHms(): TileOverlayOptions {
    Log.d("GmsMapTileOverlay", this.toString());
    val tileOverlayOptions = TileOverlayOptions()
    val mTileProvider = TileProvider { x, y, zoom ->
        Log.d("GmsMapTileOverlay", "12");
        if (tileProvider == null){
            Log.d("GmsMapTileOverlay", "is null");
        }
        val gmsTile = tileProvider.getTile(x, y, zoom)
        Log.d("GmsMapTileOverlay", "$x, $y, $zoom");
        Tile(gmsTile.width, gmsTile.height, gmsTile.data)
    }
    return tileOverlayOptions.fadeIn(fadeIn).visible(isVisible)
        .tileProvider(mTileProvider) // todo
...
fun PatternItem.toGms(): GmsPatternItem = GmsPatternItem(type, length)
