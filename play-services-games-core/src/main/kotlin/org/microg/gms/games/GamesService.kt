...
import com.google.android.gms.games.player.Player
...
class GamesServiceImpl(val context: Context, private val lifecycle: Lifecycle, val packageName: String, val account: Account, val player: Player) :
    ...
    override fun reqServerSideAccess(callbacks: IGamesCallbacks, serverClientId: String, forceRefreshToken: Boolean) {
        lifecycleScope.launchWhenStarted {
            try {
                val serverAuthTokenResponse = withContext(Dispatchers.IO) {
                    val serverAuthTokenManager = AuthManager(context, account.name, packageName, "oauth2:server:client_id:${serverClientId}:api_scope:${Scopes.GAMES_LITE}", AuthRequest().apply {
                        oauth2Prompt = if (forceRefreshToken) "consent" else "auto"
                        itCaveatTypes = "2"
                    })
                ...
                if (serverAuthTokenResponse.auth != null) {
                    callbacks.onServerAuthCodeUpdated(Status(CommonStatusCodes.SUCCESS), serverAuthTokenResponse.auth)
                } else {
                    callbacks.onServerAuthCodeUpdated(Status(CommonStatusCodes.SIGN_IN_REQUIRED), null)
                }
            } catch (e: Exception) {
                Log.w(TAG, e)
                runCatching { callbacks.onServerAuthCodeUpdated(Status(CommonStatusCodes.INTERNAL_ERROR), null) }
            }
        }
    }
