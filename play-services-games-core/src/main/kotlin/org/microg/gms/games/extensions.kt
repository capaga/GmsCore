...
import com.google.android.gms.games.player.Level
import com.google.android.gms.games.player.LevelInfo
    ...
    getJSONObject("experienceInfo")?.let {
        LevelInfo(
            ...
            it.getJSONObject("currentLevel")?.let {
                Level(it.getInt("level"), it.optLong("minExperiencePoints"), it.optLong("maxExperiencePoints"))
            },
...
suspend fun registerForGames(context: Context, account: Account, queue: RequestQueue = Volley.newRequestQueue(context)) {
    val authManager = AuthManager(context, account.name, Constants.GMS_PACKAGE_NAME, "oauth2:${Scopes.GAMES_FIRSTPARTY}", AuthRequest().apply {
        oauth2Foreground = "1"
    })
...
suspend fun performGamesSignIn(
    ...
    val authManager = AuthManager(context, account.name, packageName, "oauth2:${scopes.joinToString(" ")}", AuthRequest().apply {
        if (scopes.size == 1) itCaveatTypes = "2"
    })
