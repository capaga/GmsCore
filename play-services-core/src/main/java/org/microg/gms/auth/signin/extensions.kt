...
suspend fun performSignIn(context: Context, packageName: String, options: GoogleSignInOptions?, account: Account, permitted: Boolean = false): GoogleSignInAccount? {
    ...
    val tokenId = if (options?.isIdTokenRequested == true) idTokenResponse?.auth else null  //audienceToken
    val serverAuthCode: String? = if (options?.isServerAuthCodeRequested == true) serverAuthTokenResponse?.auth else null //oauth2token
