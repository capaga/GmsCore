...
class AuthSignInServiceImpl(
        ...
        lifecycleScope.launchWhenStarted {
            try {
                ...
                if (account != null && options?.isForceCodeForRefreshToken != true) {
                    if (AuthServiceManager.instance.getAuthManager(context, packageName, account, oAuthServerService(options)).isPermitted || AuthPrefs.isTrustGooglePermitted(context)) {
    ...
    override fun revokeAccess(callbacks: ISignInCallbacks, options: GoogleSignInOptions?) {
        ...
        lifecycleScope.launchWhenStarted {
            ...
            if (account != null) {
                try {
                    val authManager = AuthServiceManager.instance.getAuthManager(context, packageName, account, oAuthServerService(options))
