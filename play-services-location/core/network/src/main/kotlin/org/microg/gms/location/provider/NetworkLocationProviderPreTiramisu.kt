...
class NetworkLocationProviderPreTiramisu : AbstractLocationProviderPreTiramisu {
    ...
    private fun updateRequest() {
        if (enabled) {
            ...
            intent.putExtra(LocationUtil.EXTRA_PENDING_INTENT, pendingIntent)
            intent.putExtra(LocationUtil.EXTRA_ENABLE, true)
            intent.putExtra(LocationUtil.EXTRA_INTERVAL_MILLIS, intervalMillis)
            intent.putExtra(LocationUtil.EXTRA_FORCE_NOW, forceNow)
            if (SDK_INT >= 31) {
                intent.putExtra(LocationUtil.EXTRA_LOW_POWER, currentRequest?.isLowPower ?: false)
                intent.putExtra(LocationUtil.EXTRA_WORK_SOURCE, currentRequest?.workSource)
            }
            if (SDK_INT >= 29) {
                intent.putExtra(LocationUtil.EXTRA_BYPASS, currentRequest?.isLocationSettingsIgnored ?: false)
            }
    ...
    override fun enable() {
        synchronized(this) {
            ...
            intent.action = LocationUtil.ACTION_REPORT_LOCATION
