...
class LocationManager(private val context: Context, private val lifecycle: Lifecycle) : LifecycleOwner {
    ...
    fun stop() {
        ...
        if (LocationUtil.isNetWorkLocationServiceAvailable(context)) {
            val intent = Intent().apply {
                action = LocationUtil.ACTION_NETWORK_LOCATION_SERVICE
                setPackage(Constants.GMS_PACKAGE_NAME)
            }
            intent.putExtra(LocationUtil.EXTRA_PENDING_INTENT, coarsePendingIntent)
            intent.putExtra(LocationUtil.EXTRA_ENABLE, false)
    ...
    private fun onRequestManagerUpdated() {
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            ...
            if (LocationUtil.isNetWorkLocationServiceAvailable(context)) {
                ...
                intent.putExtra(LocationUtil.EXTRA_PENDING_INTENT, coarsePendingIntent)
                intent.putExtra(LocationUtil.EXTRA_ENABLE, true)
                intent.putExtra(LocationUtil.EXTRA_INTERVAL_MILLIS, networkInterval)
                intent.putExtra(LocationUtil.EXTRA_LOW_POWER, requestManager.granularity <= GRANULARITY_COARSE)
                intent.putExtra(LocationUtil.EXTRA_WORK_SOURCE, requestManager.workSource)
                context.startService(intent)
            }

            val locationManager = context.getSystemService<SystemLocationManager>() ?: return
            locationManager.requestSystemProviderUpdates(SystemLocationManager.GPS_PROVIDER, gpsInterval, gpsLocationListener)
            if (!LocationUtil.isNetWorkLocationServiceAvailable(context) && LocationManagerCompat.hasProvider(locationManager, SystemLocationManager.NETWORK_PROVIDER)) {
    ...
    fun dump(writer: PrintWriter) {
        ...
        writer.println("Network location: built-in=${LocationUtil.isNetWorkLocationServiceAvailable(context)} system=$boundToSystemNetworkLocation")
